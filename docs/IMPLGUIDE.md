# Coding Models Tracker — Implementation Guide

**Sürüm:** 1.0 | **24 Nisan 2026** | **Coding-ready** for Claude Code session
...
# 2. Skill kurulum (yerel)
mkdir -p ~/.claude/skills/coding-models-tracker
mkdir -p ~/.claude/agents

# SKILL.md ve agent.md export'tan kopyala (bkz. Project Kickstart)
...
# (Otomatik deploy, GitHub Actions gerekmez)
```
...
**Data flow:** User triggers skill → agent scrapes → validate ≥2 source → diff preview → user approve → write data/*.json → git commit → GitHub Pages auto-deploy → browser loads new data on next visit.
...
**Phase 1 — Foundation (T1-T4 + T12, paralel after T1)**

1. **T1 [30 dk]** — Repo init + GitHub Pages setup
   - `gh repo create`, push initial commit, enable Pages
   - Create folder structure, `.gitignore`, README stub
2. **T2 [1 sa]** — `data/models.json` schema + migrate from `docs/coding-models-comparison-2026-04-22.html`
   - Extract MODELS array from HTML, transform to new schema with `pricing` nested object, `lastUpdated`
3. **T3 [1 sa]** — `data/sources.json` provenance migrate
   - Per-score source attribution from existing HTML notes
4. **T4 [1.5 sa]** — `data/gpu-database.json` build
   - NVIDIA RTX 40/30/20, Apple M1-M4 tiers, AMD RX 7000, Intel Arc
5. **T12 [1.5 sa]** — `coding-models-research-agent.md` clone + specialize
   - Copy `~/.claude/agents/ledger-research-agent.md` template
...
**Phase 2 — Core (T5, T6, T13, paralel after Phase 1)**

6. **T5 [2 sa]** — `index.html` structure + `assets/app.css` 3-breakpoint responsive
   - Mobile <640px, tablet 641-1024px, desktop >1024px
   - Copy CSS variables from existing HTML report (`--bg`, `--surface`, `--accent`, etc.)
7. **T6 [1 sa]** — `assets/app.js` data fetch + render MODELS
   - `loadData()` (see §4.3), render to `<table>` or `<div class="models-grid">`
   - Reactive: data load → render → user interactions trigger re-render
8. **T13 [2 sa]** — `~/.claude/skills/coding-models-tracker/SKILL.md` orchestrator
...
**Phase 3 — Features (T7-T11, T15, paralel after T6)**

9. **T7 [2 sa]** — Dynamic weights editor UI
   ...
   - 4 preset buttons: SWE-odaklı, Agentic-odaklı, Balanced, Benchmark-only
   - Reset-to-default button
   - localStorage persist: `cmt.v1.weights`
10. **T8 [1.5 sa]** — i18n TR/EN switch
...
13. **T11 [2 sa]** — GPU VRAM detection + filter
    ...
    - Filter UI: `<input type="checkbox">` "Sadece GPU'mda çalışanları göster"
    - Per local model: compatibility badge (✓ ≤vram-1, ⚠ ≤vram (offload), ✗ >vram)
    - Unsloth UD priority: en yüksek-quality variant ≤ user vram seç, recommend
    - localStorage persist: `cmt.v1.vram`
14. **T15 [1 sa]** — Skill validation logic
    - In `SKILL.md`: validate agent output (≥2 source per score, ≥%95 coverage, contradiction detect)
    - Block release if coverage < 95%, show user warning + force-override option
    - >5pp contradiction: red flag + user manual pick required

**Phase 4 — Polish (paralel)**

15. **T14 [30 dk]** — Skill install script + docs (`README.md` skill section)
16. **T21 [1 sa]** — SEO: meta tags + Open Graph + JSON-LD (Article + Dataset) + hreflang + sitemap.xml + robots.txt
17. **T22 [30 dk]** — `CHANGELOG.md` bootstrap + format convention (Keep a Changelog style)

**Phase 5 — Test (sync gate)**

18. **T23 [2-3 sa]** — End-to-end: tüm 13 AC pass
    - Manuel test plan: weights total constraint, contradiction render, GPU filter, PNG export iOS Safari + Chrome Android, responsive 3 breakpoint, i18n switch, skill invocation E2E

**Phase 6 — Content + Launch (paralel after T23)**

19. **T16 [1 sa]** — README.md TR+EN + skill installation guide
20. **T17 [2 sa]** — 2 launch-day CP1 analysis post (DeepSeek V4 + Qwen3.6-27B derin analizi)
21. **T18 [2 sa]** — 1 CP2 evergreen rehber: "8GB VRAM dev için top 5 coding model" (QW1)
22. **T19 [3 sa]** — 1 CP3 quarterly: "SWE-bench Verified vs Pro: %35 puanlık fark neden?" (QW2)
23. **T20 [1.5 sa]** — Launch playbook: kanal listesi + ready-to-post text TR+EN
...
|-------|----------|-----------|--------|---------------|
| Agent | HTTP fetch fail | catch network error | Retry 1× → fallback WebSearch → user "kısmi veri, devam?" | Yes (skill prompt) |
...
| Skill | User decline diff | user input "no" | Rollback (no file write, no commit) | Yes (confirmation) |
...
| Browser | html2canvas fail | catch promise reject | Alert: "Export failed, sayfa yenileyin" | Yes |
| Browser | i18n fetch fail | catch network error | Fallback to TR (default), console warn | No |
| GitHub Pages | Deploy delay >5dk | manuel check post-push | Skill warns user, suggest GitHub status check | Yes |
...
3. Wait ~1-2 dk for first deploy
...
3. GitHub Pages auto-deploy (~1-2 dk)
4. Verify M5 metric (≤14 gün max update aralık) via git log

### No-build verification
- ❌ No `package.json`, `npm`, `node_modules`
- ❌ No CI/CD workflow (default Pages deploy yeterli)
...
```javascript
const CONTRADICTION_WARN = 3.0;  // ≥3pp delta → ⚠ flag
...
```javascript
const VALIDATION_COVERAGE_MIN = 0.95;  // ≥%95 benchmark must have ≥2 source (M4 metric)
const STALE_THRESHOLD_DAYS = 14;       // M5 metric: ≤14 gün max update interval
```
...
|---------|---------|-----|--------|
| html2canvas | 1.4.1 (Apr 2024 latest) | https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js | manuel verify on download |

**Action:** Run `Agent` with `scope: dependency` to verify latest stable before T10 implementation.
...
| AC | Test |
|----|------|
| AC1 Agent research | Run skill, verify ≥15 source fetch + structured JSON return |
| AC2 ≥2 source coverage | Inspect `validationCoverage`, must ≥0.95 |
...
| AC5 GitHub Pages deploy | Push commit, verify live in <2 dk + JSON fetch <2sn |
| AC-F1 Skill+agent | Invoke `/ledger-tracker-update`, verify <5sn agent start |
...
| AC-F12 Contradiction render | Verify 3pp ⚠, 5pp 🚨, tooltip source breakdown |
...
- [ ] All 13 AC E2E test passes
- [ ] Lighthouse audit ≥90 all categories
- [ ] M5 launch playbook ready (`T20` deliverable)
- [ ] 3 launch-day posts drafted (DeepSeek V4 + Qwen3.6-27B + 8GB VRAM rehberi)
