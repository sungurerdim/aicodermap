...
|----|----------|----------|
| AC1 | First load with empty `localStorage` | TR locale if browser lang starts with `tr`, otherwise EN. Dark theme. Default weights sum 100. All active models render with `loading` placeholder gone. |
...
- All active models render with no console errors
